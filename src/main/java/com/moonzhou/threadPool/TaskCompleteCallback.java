...
public class TaskCompleteCallback {
	...
	/**
	 * <一> 1. 用List收集任务结果 (List记录每个submit返回的Future)
	 * 
	 * 2. 循环查看结果, Future不一定完成, 如果没有完成, 那么调用get会阻塞
	 * 
	 * 3. 如果排在前面的任务没有完成, 那么就会阻塞, 这样后面已经完成的任务就没法获得结果了, 导致了不必要的等待时间. 更为严重的是: 第一个任务如果几个小时或永远完成不了, 而后面的任务几秒钟就完成了, 那么后面的任务的结果都将得不到处理
	 *
	 * 导致: 已完成的任务可能得不到及时处理
	 */
	...
	/**
	 * <三> 使用ExecutorCompletionService管理异步任务 1. Java中的ExecutorCompletionService<V>本身有管理任务队列的功能 i. ExecutorCompletionService内部维护列一个队列, 用于管理已完成的任务 ii. 内部还维护列一个Executor, 可以执行任务
	 *
	 ...
	 *
	 * 4. ExecutorCompletionService的take/poll方法是对BlockingQueue对应的方法的封装, 关于BlockingQueue的take/poll方法: i. take()方法, 如果队列中有数据, 就返回数据, 否则就一直阻塞; ii. poll()方法: 如果有值就返回, 否则返回null iii. poll(long timeout, TimeUnit unit)方法: 如果有值就返回, 否则等待指定的时间; 如果时间到了如果有值, 就返回值, 否则返回null
	 *
	 * 解决了已完成任务得不到及时处理的问题
	 */
